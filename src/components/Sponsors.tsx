...
const SponsorTier = ({ title, sponsors, delay }: { title: string; sponsors: Sponsor[]; delay: number }) => {
  return (
    <motion.div
      ...
      className="mb-12"
    >
      <h3 className="font-heading font-bold text-2xl text-white text-center mb-8">{title}</h3>
          ...
          >
            <div className="relative w-full h-full flex items-center justify-center">
              ...
              <div className="w-12 h-12 bg-gradient-to-r from-[var(--accent-pink)] to-[var(--accent-cyan)] rounded-lg flex items-center justify-center">
                <span className="text-black font-bold text-xl">{sponsor.name.charAt(0)}</span>
              </div>
            </div>
          </motion.div>
        ))}
...
const Sponsors = () => {
  ...
  return (
    <section id="sponsors" className="py-20 px-6 bg-[var(--bg-mid)] relative overflow-hidden">
      {/* Background Elements */}
      ...
      <div className="container mx-auto max-w-6xl relative z-10" ref={ref}>
        ...
        >
          ...
          <p className="font-body text-lg md:text-xl text-white/80 max-w-2xl mx-auto">
            We're grateful to our amazing sponsors who make The Flow Party possible
          </p>
        </motion.div>
        ...
        {isInView && (
          <div>
            {titleSponsors.length > 0 && (
              <SponsorTier title="Title Sponsor" sponsors={titleSponsors} delay={0.2} />
            )}
            
            {platinumSponsors.length > 0 && (
              <SponsorTier title="Platinum Partners" sponsors={platinumSponsors} delay={0.4} />
            )}
            
            {goldSponsors.length > 0 && (
              <SponsorTier title="Gold Sponsors" sponsors={goldSponsors} delay={0.6} />
            )}
            
            {silverSponsors.length > 0 && (
              <SponsorTier title="Silver Sponsors" sponsors={silverSponsors} delay={0.8} />
            )}
            
            {bronzeSponsors.length > 0 && (
              <SponsorTier title="Bronze Sponsors" sponsors={bronzeSponsors} delay={1.0} />
            )}
          </div>
        )}
        ...
        >
          <div className="glass rounded-2xl p-8">
            ...
            <p className="text-white/70 font-body mb-6 max-w-2xl mx-auto">
              Join our community of forward-thinking brands and reach thousands of engaged attendees. 
              Partner with us to create unforgettable experiences.
            </p>
            <motion.button
