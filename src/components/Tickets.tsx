...
const TicketModal = ({ isOpen, onClose, ticketName }: { isOpen: boolean; onClose: () => void; ticketName: string }) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    alert(`Thank you! Your ${ticketName} ticket(s) have been reserved. Check your email for confirmation.`);
    onClose();
  };
        ...
        >
          ...
          >
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              <div>
                <label className="block text-white/80 font-body text-sm mb-2">Full Name</label>
                <input
                  type="text"
                  required
                  value={formData.name}
              ...
              <div>
                <label className="block text-white/80 font-body text-sm mb-2">Email Address</label>
                <input
                  type="email"
                  required
              ...
              <div>
                <label className="block text-white/80 font-body text-sm mb-2">Quantity</label>
                <select
                ...
                >
                  {[1,2,3,4,5].map(num => (
                    <option key={num} value={num} className="bg-[var(--bg-dark)]">{num}</option>
                  ))}
                </select>
              </div>
...
const TicketCard = ({ ticket, index }: { ticket: TicketTier; index: number }) => {
  ...
  return (
    <>
      <motion.div
        ...
        className={`relative ${ticket.highlighted ? 'transform scale-105' : ''}`}
      >
        {ticket.highlighted && (
          <motion.div
...
const Tickets = () => {
  ...
  return (
    <section id="tickets" className="py-20 px-6 bg-[var(--bg-dark)] relative overflow-hidden">
      {/* Background Elements */}
      ...
      <div className="container mx-auto max-w-7xl relative z-10" ref={ref}>
        ...
        >
          <p className="text-white/60 font-body text-sm">
            All tickets include event entry, welcome drink, and access to main stage area.<br/>
